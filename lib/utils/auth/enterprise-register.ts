...
export function validateEnterpriseRegisterForm(args: {
  ...
  if (!ENTERPRISE_ADDRESS_PATTERN.test(form.addressDetail)) {
  ...
  if (!ENTERPRISE_LETTER_ONLY_PATTERN.test(form.representativeName)) {
    nextErrors.representativeName = "Họ và tên chỉ gồm ký tự chữ, dài 1-255.";
  }
  if (!ENTERPRISE_LETTER_ONLY_PATTERN.test(form.representativeTitle)) {
    nextErrors.representativeTitle = "Chức vụ chỉ gồm ký tự chữ, dài 1-255.";
  }
  if (!/^\d{8,12}$/.test(form.phone)) {
    nextErrors.phone = "Số điện thoại chỉ gồm số, dài 8-12 ký tự.";
  }
  if (!ENTERPRISE_EMAIL_REGEX.test(form.email)) {
    nextErrors.email = "Email phải đúng định dạng example@domain.com.";
  }
