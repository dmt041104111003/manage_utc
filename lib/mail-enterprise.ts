...
/** Tiêu đề & nội dung theo đặc tả phê duyệt. */
export async function sendEnterpriseApprovedEmail(to: string, companyName: string, loginEmail: string) {
  ...
  const greeting = companyName.trim() ? `Kính gửi ${companyName},` : "Kính gửi Quý Doanh nghiệp,";
  const subject =
    "Phòng đào tạo Trường Đại học GTVT - Thông báo phê duyệt tài khoản kết nối thực tập thành công";
  const lines = [
    greeting,
    "",
    `Thay mặt ${ENTERPRISE_MAIL_SIGN_OFF_SCHOOL}, chúng tôi xin trân trọng thông báo hồ sơ đăng ký tham gia hệ thống Quản lý thực tập của Quý doanh nghiệp đã được phê duyệt thành công.`,
    "",
...
export async function sendEnterpriseRejectedEmail(
  ...
  const subject =
    "Phòng đào tạo Trường Đại học GTVT - Thông báo kết quả đăng ký tài khoản kết nối thực tập";
  const greeting = companyName.trim() ? `Kính gửi ${companyName},` : "Kính gửi Quý Doanh nghiệp,";
  const reasonBlock =
    reasons.length > 0
  ...
  const lines = [
    greeting,
    "",
    `Lời đầu tiên, ${ENTERPRISE_MAIL_SIGN_OFF_SCHOOL} xin chân thành cảm ơn Quý doanh nghiệp đã quan tâm và gửi hồ sơ đăng ký tham gia hệ thống quản lý thực tập của nhà trường.`,
    "",
    ...
    "",
    "Hướng dẫn xử lý:",
    `- Nếu lý do liên quan đến hồ sơ hoặc thông tin sai sót, Quý doanh nghiệp vui lòng truy cập lại trang đăng ký tại địa chỉ ${registerLink} để cập nhật lại thông tin chính xác (nếu tài khoản vẫn tồn tại, vui lòng liên hệ Phòng đào tạo).`,
